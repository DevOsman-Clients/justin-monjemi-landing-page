...
export default function AchievementsPage() {
  ...
  const achievements = [
    ...
    },
    {
      ...
      description:
        language === "tr" ? "Öğrencilerimiz önemli burslar kazandı" : "Our students secured significant scholarships",
      number: "£2M+",
    },
    {
      icon: Target,
      title: language === "tr" ? "Vize Başarısı" : "Visa Success",
      description: language === "tr" ? "Vize başvurularında %98 başarı oranı" : "98% success rate in visa applications",
      number: "98%",
    },
  ]
  ...
  const prevStory = () => {
    setCurrentStoryIndex((prev) => (prev - 3 + successStories.length) % successStories.length)
  }

  const getVisibleStories = () => {
    const stories = []
    for (let i = 0; i < 3; i++) {
      stories.push(successStories[(currentStoryIndex + i) % successStories.length])
    }
    return stories
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative py-20 bg-gradient-to-br from-[#D29D33] to-[#b8851f]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-6">{content.achievements.title}</h1>
          <p className="text-xl text-white/90 max-w-3xl mx-auto">{content.achievements.subtitle}</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {content.achievements.stats.map((stat, index) => (
              <Card key={index} className="text-center p-8 border-none shadow-lg hover:shadow-xl transition-shadow">
                <CardContent className="pt-6">
                  <div className="text-4xl font-bold text-[#D29D33] mb-2">{stat.number}</div>
                  <div className="text-lg font-semibold text-[#032445]">{stat.label}</div>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-center text-[#032445] mb-16">
            {language === "tr" ? "Detaylı Başarılarımız" : "Our Detailed Achievements"}
          </h2>
          <div className="grid md:grid-cols-2 gap-8">
            {achievements.map((achievement, index) => (
              <Card key={index} className="p-8 border-none shadow-lg hover:shadow-xl transition-shadow">
                <CardContent className="pt-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <div className="text-3xl font-bold text-[#D29D33] mb-2">{achievement.number}</div>
                      <h3 className="text-xl font-bold text-[#032445] mb-3">{achievement.title}</h3>
              ...
              {getVisibleStories().map((story, index) => (
                ...
                >
                  ...
                  <CardContent className="p-6">
                    <h3 className="text-xl font-bold text-[#032445] mb-2">{story.name}</h3>
                    <p className="text-[#D29D33] font-semibold mb-1">{story.university}</p>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-semibold text-[#032445]">
                        {language === "tr" ? "Başarılı Kabul" : "Successful Admission"}
                      </span>
                    </div>
                  </CardContent>
                </Card>
              ...
              {Array.from({ length: Math.ceil(successStories.length / 3) }).map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-colors ${
                    Math.floor(currentStoryIndex / 3) === index ? "bg-[#D29D33]" : "bg-gray-300"
                  }`}
                />
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 items-center opacity-80">
              {["UK Department for Education", "British Council", "UKVI Approved Agent", "GDPR Compliant"].map(
                (cert, index) => (
